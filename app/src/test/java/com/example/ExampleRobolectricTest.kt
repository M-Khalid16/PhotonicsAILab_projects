...
class ExampleRobolectricTest {
  ...
  fun `read string from context`() {
    ...
    assertEquals("FacultyAid", appName)
  }

  @Test
  fun `initialize FacultyViewModel successfully`() {
    val context = ApplicationProvider.getApplicationContext<Context>() as android.app.Application
    val viewModel = FacultyViewModel(context)
    assertNotNull(viewModel)
    assertNotNull(viewModel.profileState.value)
  }
